...
include_once( 'settings/class-wc-uprisepay-cards-settings.php' );
include_once( 'settings/class-wc-uprisepay-webhook.php' );
include_once( 'settings/admin/class-wc-uprisepay-admin.php' );
include_once( 'api/class-wc-uprisepay-api-request.php' );
include_once( 'class-wc-uprise-payment-webhook.php' );
include_once( 'subscription/class-wc-uprisepay-subscription.php' );

/**
 * Class WC_Gateway_Uprise_Payment_Cards for Card payment method.
 */
class WC_Gateway_Uprise_Payment_Cards extends WC_Payment_Gateway_CC {

	/**
	 * WC_Gateway_Uprise_Payment_Cards constructor.
	 */
	public function __construct() {
		$this->id                 = 'wc_uprise_payment_cards';
		...
		$this->title              = __( 'Cards payment and general configuration', 'uprise-payment-woocommerce' );
		...
		$this->new_method_label = __( 'Use a new card', 'uprise-payment-woocommerce' );
		...
		// Redirection hook.
		add_action( 'woocommerce_api_wc_uprisepay_callback', [ $this, 'callback_handler' ] );

		// Webhook handler hook.
		add_action( 'woocommerce_api_wc_uprisepay_webhook', [ $this, 'webhook_handler' ] );
	...
	public function payment_scripts() {
		...
		// Styles.
		if ( WC_Admin_Settings::get_option( 'upycom_iframe_style', '0' ) ) {
		...
		$vars = [
			'card-number'           => esc_html__( 'Please enter a valid card number', 'uprise-payment-woocommerce' ),
			'expiry-date'           => esc_html__( 'Please enter a valid expiry date', 'uprise-payment-woocommerce' ),
			'cvv'                   => esc_html__( 'Please enter a valid cvv code', 'uprise-payment-woocommerce' ),
			'is-add-payment-method' => is_wc_endpoint_url( 'add-payment-method' ) ? 'yes' : 'no',
		];

		wp_localize_script( 'cko-frames-script', 'upy_frames_vars', $vars );
		...
		$this->form_fields = array_merge(
			...
			[
				'screen_button' => [
					'id'    => 'screen_button',
					'type'  => 'screen_button',
					'title' => __( 'Other Settings', 'uprise-payment-woocommerce' ),
				],
			]
		);
	}
		...
		if ( ! isset( $_GET['screen'] ) || '' === sanitize_text_field( $_GET['screen'] ) ) {
			...
			$test = [
				'screen_button' => [
					'id'    => 'screen_button',
					'type'  => 'screen_button',
					'title' => __( 'Settings', 'uprise-payment-woocommerce' ),
				],
			];
	...
	public function payment_fields() {
		$save_card             = WC_Admin_Settings::get_option( 'upycom_card_saved' );
		$mada_enable           = '1' === WC_Admin_Settings::get_option( 'upycom_card_mada', '0' );
		$require_cvv           = WC_Admin_Settings::get_option( 'upycom_card_require_cvv' );
		$is_mada_token         = false;
		$card_validation_alert = __( 'Please enter your card details.', 'uprise-payment-woocommerce' );
		$iframe_style          = WC_Admin_Settings::get_option( 'upycom_iframe_style', '0' );

		?>
		<input type="hidden" id="debug" value='<?php echo WC_Admin_Settings::get_option( 'upy_console_logging' ); ?>' />
		<input type="hidden" id="public-key" value='<?php echo $this->get_option( 'upycom_pk' ); ?>'/>
				...
				<div class="multi-frame">
					<div class="input-container card-number">
						<div class="icon-container">
							<img id="icon-card-number" src="<?php echo esc_url( WC_CHECKOUTCOM_PLUGIN_URL . '/assets/images/card-icons/card.svg' ); ?>" alt="<?php esc_attr_e( 'PAN', 'uprise-payment-woocommerce' ); ?>"/>
						</div>
						...
						<div class="icon-container">
							<img id="icon-card-number-error" src="<?php echo esc_url( WC_CHECKOUTCOM_PLUGIN_URL . '/assets/images/card-icons/error.svg' ); ?>" alt="<?php esc_attr_e( 'PAN error', 'uprise-payment-woocommerce' ); ?>"/>
						</div>
					</div>
					...
					<div class="date-and-code">
						<div>
							<div class="input-container expiry-date">
								<div class="icon-container">
									<img id="icon-expiry-date" src="<?php echo esc_url( WC_CHECKOUTCOM_PLUGIN_URL . '/assets/images/card-icons/exp-date.svg' ); ?>" alt="<?php esc_attr_e( 'Expiry date', 'uprise-payment-woocommerce' ); ?>" />
								</div>
								<div class="expiry-date-frame"></div>
								<div class="icon-container">
									<img id="icon-expiry-date-error" src="<?php echo esc_url( WC_CHECKOUTCOM_PLUGIN_URL . '/assets/images/card-icons/error.svg' ); ?>" alt="<?php esc_attr_e( 'Expiry error', 'uprise-payment-woocommerce' ); ?>"/>
								</div>
							</div>
						</div>
						...
						<div>
							<div class="input-container cvv">
								<div class="icon-container">
									<img id="icon-cvv" src="<?php echo esc_url( WC_CHECKOUTCOM_PLUGIN_URL . '/assets/images/card-icons/cvv.svg' ); ?>" alt="<?php esc_attr_e( 'CVV', 'uprise-payment-woocommerce' ); ?>" />
								</div>
								<div class="cvv-frame"></div>
								<div class="icon-container">
									<img id="icon-cvv-error" src="<?php echo esc_url( WC_CHECKOUTCOM_PLUGIN_URL . '/assets/images/card-icons/error.svg' ); ?>" alt="<?php esc_attr_e( 'CVV error', 'uprise-payment-woocommerce' ); ?>" />
								</div>
							</div>
						</div>
					</div>
	...
	public function element_form_cvv() {
		...
		<div class="cko-cvv" style="display: none;padding-top: 10px;">
			<p class="validate-required" id="cko-cvv" data-priority="10">
				<label for="cko-cvv"><?php esc_html_e( 'Card Code', 'uprise-payment-woocommerce' ); ?> <span class="required">*</span></label>
				<input id="cko-cvv" type="text" autocomplete="off" class="input-text" placeholder="<?php esc_attr_e( 'CVV', 'uprise-payment-woocommerce' ); ?>" name="<?php echo esc_attr( $this->id ); ?>-card-cvv"/>
			</p>
		</div>
		<?php
	}
	...
	public function process_payment( $order_id ) {
		...
		if ( sanitize_text_field( $_POST['wc-wc_uprise_payment_cards-payment-token'] ) ) {
			if ( 'new' === sanitize_text_field( $_POST['wc-wc_uprise_payment_cards-payment-token'] ) ) {
				$arg = sanitize_text_field( $_POST['cko-card-token'] );
			} else {
				$arg = sanitize_text_field( $_POST['wc-wc_uprise_payment_cards-payment-token'] );
			}
		}
		...
		if ( empty( $arg ) ) {
			...
			if ( sanitize_text_field( $_POST['cko-card-token'] ) ) {
				...
				WC_Checkoutcom_Utility::wc_add_notice_self( __( 'There was an issue completing the payment.', 'uprise-payment-woocommerce' ), 'error' );
		...
		if ( class_exists( 'WC_Subscriptions_Order' ) ) {
			...
			if ( ! empty( $card_scheme ) ) {
				WC()->session->set( '_upy_preferred_scheme', $card_scheme );
			}
		}
		...
		if ( isset( $result['redirectLink'] ) && ! empty( $result['redirectLink'] ) ) {
			...
			if ( $save_card && isset( $_POST['wc-wc_uprise_payment_cards-new-payment-method'] ) && sanitize_text_field( $_POST['wc-wc_uprise_payment_cards-new-payment-method'] ) ) {
				// Save in session for 3D secure payment.
				WC()->session->set( 'wc-wc_uprise_payment_cards-new-payment-method', 'yes' );
			} else {
				WC()->session->set( 'wc-wc_uprise_payment_cards-new-payment-method', 'no' );
			}
		...
		if ( class_exists( 'WC_Subscriptions_Order' ) ) {
			...
			$card_scheme = WC()->session->get( '_upy_preferred_scheme' );

			if ( ! empty( $card_scheme ) ) {
				$this->save_preferred_card_scheme( $order_id, $order, $card_scheme );
				WC()->session->__unset( '_upy_preferred_scheme' );
			}
		}
		...
		$message = sprintf( esc_html__( 'Uprise Payment Authorised - Action ID : %s', 'uprise-payment-woocommerce' ), $result['action_id'] );

		// Check if payment was flagged.
		if ( $result['risk']['flagged'] ) {
			// Get cko auth status configured in admin.
			$status = WC_Admin_Settings::get_option( 'upycom_order_flagged', 'flagged' );

			/* translators: %s: Action ID. */
			$message = sprintf( esc_html__( 'Uprise Payment Flagged - Action ID : %s', 'uprise-payment-woocommerce' ), $result['action_id'] );
		}
	...
	public function callback_handler() {
		...
		if ( $_REQUEST['cko-session-id'] ) {
			$upy_session_id = $_REQUEST['cko-session-id'];
		}

		// Verify session id.
		$result = (array) ( new WC_Checkoutcom_Api_Request )->verify_session( $upy_session_id );
		...
		if ( 'error' === $result['card_verification'] ) {
			WC_Checkoutcom_Utility::wc_add_notice_self( __( 'Unable to add payment method to your account.', 'uprise-payment-woocommerce' ), 'error' );
		...
		if ( 'Card Verified' === $result['status'] && isset( $result['metadata']['card_verification'] ) ) {
			...
			WC_Checkoutcom_Utility::wc_add_notice_self( __( 'Payment method successfully added.', 'uprise-payment-woocommerce' ), 'notice' );
		...
		/* translators: %s: Action ID. */
		$message = sprintf( esc_html__( 'Uprise Payment Authorised - Action ID : %s', 'uprise-payment-woocommerce' ), $action['0']['id'] );
		...
		if ( $result['risk']['flagged'] ) {
			...
			/* translators: %s: Action ID. */
			$message = sprintf( esc_html__( 'Uprise Payment Flagged - Action ID : %s', 'uprise-payment-woocommerce' ), $action['0']['id'] );
		}
		...
		if ( 'Canceled' === $result['status'] ) {
			...
			/* translators: %s: Action ID. */
			$message = sprintf( esc_html__( 'Uprise Payment Canceled - Action ID : %s', 'uprise-payment-woocommerce' ), $action['0']['id'] );
		}

		if ( 'Captured' === $result['status'] ) {
			update_post_meta( $order_id, 'upy_payment_captured', true );
			$status = WC_Admin_Settings::get_option( 'upycom_order_captured', 'processing' );

			/* translators: %s: Action ID. */
			$message = sprintf( esc_html__( 'Uprise Payment Captured - Action ID : %s', 'uprise-payment-woocommerce' ), $action['0']['id'] );
		}

		// save card to db.
		$save_card = WC_Admin_Settings::get_option( 'upycom_card_saved' );
		if ( $save_card && 'yes' === WC()->session->get( 'wc-wc_uprise_payment_cards-new-payment-method' ) ) {
			$this->save_token( $order->get_user_id(), $result );
			WC()->session->__unset( 'wc-wc_uprise_payment_cards-new-payment-method' );
		}
	...
	public function add_payment_method() {
		...
		$gateway_debug = 'yes' === WC_Admin_Settings::get_option( 'upy_gateway_responses', 'no' );
		...
		$redirection_url      = add_query_arg( 'wc-api', 'wc_uprisepay_callback', home_url( '/' ) );
		...
		try {
			...
			$error_message = esc_html__( 'An error has occurred while processing your cancel request. ', 'uprise-payment-woocommerce' );
	...
	public function save_preferred_card_scheme( $order_id, $order, $card_scheme ) {
		...
		if ( $order instanceof WC_Subscription && ! empty( $card_scheme ) ) {
			update_post_meta( $order->get_id(), '_upy_preferred_scheme', $card_scheme );
		}
		...
		if ( function_exists( 'wcs_order_contains_subscription' ) ) {
			if ( wcs_order_contains_subscription( $order_id ) ) {
				...
				foreach ( $subscriptions as $subscription_obj ) {
					update_post_meta( $subscription_obj->get_id(), '_upy_preferred_scheme', $card_scheme );
				}
			}
		}
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		// Set action id as woo transaction id.
		update_post_meta( $order_id, '_transaction_id', $result['action_id'] );
		update_post_meta( $order_id, 'upy_payment_refunded', true );

		/* translators: %s: Action ID. */
		$message = sprintf( esc_html__( 'Uprise Payment refunded from Admin - Action ID : %s', 'uprise-payment-woocommerce' ), $result['action_id'] );

		if ( isset( $_SESSION['cko-refund-is-less'] ) ) {
			if ( $_SESSION['cko-refund-is-less'] ) {
				/* translators: %s: Action ID. */
				$order->add_order_note( sprintf( esc_html__( 'Uprise Payment Partially refunded from Admin - Action ID : %s', 'uprise-payment-woocommerce' ), $result['action_id'] ) );
	...
	public function webhook_handler() {
		// webhook_url_format = http://localhost/wordpress-5.0.2/wordpress/?wc-api=wc_uprisepay_webhook .
		...
		$core_settings['upycom_sk'] = upy_is_nas_account() ? 'Bearer ' . $core_settings['upycom_sk'] : $core_settings['upycom_sk'];

		$signature = WC_Checkoutcom_Utility::verify_signature( $raw_event, $core_settings['upycom_sk'], $header_signature );
		...
		$payment_id = get_post_meta( $data->data->metadata->order_id, '_upy_payment_id', true );
		...
		if ( $payment_id !== $data->data->id ) {
			...
			if ( $gateway_debug ) {
				...
				$message = sprintf( esc_html__( 'Order payment Id (%1$s) does not match that of the webhook (%2$s)', 'uprise-payment-woocommerce' ), $payment_id, $data->data->id );
		...
		switch ( $event_type ) {
			case 'card_verified':
				$response = WC_Uprise_Payment_Webhook::card_verified( $data );
				break;
			case 'payment_approved':
				$response = WC_Uprise_Payment_Webhook::authorize_payment( $data );
				break;
			case 'payment_captured':
				$response = WC_Uprise_Payment_Webhook::capture_payment( $data );
				break;
			case 'payment_voided':
				$response = WC_Uprise_Payment_Webhook::void_payment( $data );
				break;
			case 'payment_capture_declined':
				$response = WC_Uprise_Payment_Webhook::capture_declined( $data );
				break;
			case 'payment_refunded':
				$response = WC_Uprise_Payment_Webhook::refund_payment( $data );
				break;
			case 'payment_canceled':
				$response = WC_Uprise_Payment_Webhook::cancel_payment( $data );
				break;
			case 'payment_declined':
				$response = WC_Uprise_Payment_Webhook::decline_payment( $data );
				break;
	...
	public function get_localisation() {
		...
		switch ( $locale ) {
			...
			case 'es':
				...
				break;
			default:
				$localization = WC_Admin_Settings::get_option( 'upycom_language_fallback', 'EN-GB' );
		}
