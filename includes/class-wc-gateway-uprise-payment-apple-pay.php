...
/**
 * Class WC_Gateway_Uprise_Payment_Apple_Pay for Apple Pay method.
 */
class WC_Gateway_Uprise_Payment_Apple_Pay extends WC_Payment_Gateway {

	/**
	 * WC_Gateway_Uprise_Payment_Apple_Pay constructor.
	 */
	public function __construct() {
		$this->id                 = 'wc_uprise_payment_apple_pay';
		...
		$this->title              = __( 'Apple Pay', 'uprise-payment-woocommerce' );
		...
		// Redirection hook.
		add_action( 'woocommerce_api_wc_uprisepay_session', [ $this, 'applepay_sesion' ] );

		add_action( 'woocommerce_api_wc_uprisepay_generate_token', [ $this, 'applepay_token' ] );
	...
	public function payment_fields() {
		...
		$session_url        = str_replace( 'https:', 'https:', add_query_arg( 'wc-api', 'wc_uprisepay_session', home_url( '/' ) ) );
		$generate_token_url = str_replace( 'https:', 'https:', add_query_arg( 'wc-api', 'wc_uprisepay_generate_token', home_url( '/' ) ) );
		$apple_settings     = get_option( 'woocommerce_wc_uprise_payment_apple_pay_settings' );
		...
		<!-- ApplePay warnings -->
		<p style="display:none" id="upycom_applePay_not_actived">ApplePay is possible on this browser, but not currently activated.</p>
		<p style="display:none" id="upycom_applePay_not_possible">ApplePay is not available on this browser</p>

		<script type="text/javascript">
			// Magic strings used in file
			var applePayOptionSelector = 'li.payment_method_wc_uprise_payment_apple_pay';
			var applePayButtonId = 'upycom_applePay';

			// Warning messages for ApplePay
			var applePayNotActivated = document.getElementById('upycom_applePay_not_actived');
			var applePayNotPossible = document.getElementById('upycom_applePay_not_possible');
			...
			if (window.ApplePaySession) {
				var promise = ApplePaySession.canMakePaymentsWithActiveCard("<?php echo $this->get_option( 'upycom_apple_mercahnt_id' ); ?>");
			...
			checkoutInitialiseApplePay = function () {
				...
				+ "<?php echo $this->get_option( 'upycom_apple_type' ); ?>" + " "
				+ "<?php echo $this->get_option( 'upycom_apple_theme' ); ?>"  + '" lang="'
				+ "<?php echo $this->get_option( 'upycom_apple_language' ); ?>" + '"></div>');

				jQuery('#upycom_applePay').hide();
			};
			...
			*/
			...
			function performAppleUrlValidation(valURL, callback) {
				jQuery.ajax({
					...
					data: {
						url: valURL,
						merchantId: "<?php echo $this->get_option( 'upycom_apple_mercahnt_id' ); ?>",
			...
			/**
			 * Generate the Uprise token based on the Apple Pay payload.
			 *
			 * @param {function} callback
			 */
			...
			*/
			function showAppleApplePayOption() {
				...
				if ( jQuery( '.payment_method_wc_uprise_payment_apple_pay' ).is( ':visible' ) ) {

					//check if Apple Pay method is checked.
					if ( jQuery( '#payment_method_wc_uprise_payment_apple_pay' ).is( ':checked' ) ) {
						// Disable place order button.
						// jQuery('#place_order').prop("disabled",true);
						jQuery( '#place_order' ).hide();
						// Show Apple Pay button.
						jQuery( '#upycom_applePay' ).show();
					} else {
					...
					jQuery( "input[name='payment_method']" ).on( 'click', function () {
						...
						if ( this.value == 'wc_uprise_payment_apple_pay' ) {
	...
	public function applepay_sesion() {
		...
		$merchant_id     = $this->get_option( 'upycom_apple_mercahnt_id' );
		$certificate     = $this->get_option( 'upycom_apple_certificate' );
		$certificate_key = $this->get_option( 'upycom_apple_key' );
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if ( isset( $_SESSION['cko-refund-is-less'] ) ) {
			if ( $_SESSION['cko-refund-is-less'] ) {
				/* translators: %s: Action ID. */
				$order->add_order_note( sprintf( __( 'Uprise Payment Partially refunded from Admin - Action ID : %s', 'uprise-payment-woocommerce' ), $result['action_id'] ) );
		...
		/* translators: %s: Action ID. */
		$order->add_order_note( sprintf( __( 'Uprise Payment refunded from Admin - Action ID : %s', 'uprise-payment-woocommerce' ), $result['action_id'] ) );
